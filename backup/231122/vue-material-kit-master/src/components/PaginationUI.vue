    ...
    <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            ...
            <li
                ...
                :key="index"
                v-bind:class="{ active: startPageIndex + index - 1 == boardStore.currentPageIndex }"
                class="page-item"
            >
...
const { boardStore, startPageIndex, endPageIndex, prev, next, pageCount } = useBoardStore();

const emit = defineEmits(["call-parent"]);
const paginationChanged = (pageIndex) => {
    console.log(boardStore.totalListItemCount + "개의 글");
    console.log(pageCount + "개의 페이지가 필요");
    console.log(boardStore.pageLinkCount + "개의 페이지가 필요");
